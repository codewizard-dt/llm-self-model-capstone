...
**The architecture bridge document** (`self_model_generator/docs/llm_critic_architecture.md`, owner: Grace Huang) defines the Generator/Critic architecture: one Generator LLM and three stateless Critics (physics, torque, CoM/geometry). It now points to the deterministic first F8/F9 loop-closure slice in `self_model_generator.loop_closure`; broader residual coverage, planted-fault critic tests, and external LLM prompt adapters remain future work.
...
- `self_model_generator/` pins Python ≥3.12,<3.13.
