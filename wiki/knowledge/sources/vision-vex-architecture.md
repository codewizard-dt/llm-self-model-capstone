...
**The Pi vision stack**: OpenCV (30+ FPS color/blob detection, zero deps), Ultralytics YOLO11n with NCNN quantization (~8–10 FPS at 640×480 on Pi 5 CPU alone; 25+ FPS at 240×240), and the `apriltag` Python library (`pip install apriltag`) for workspace localization — a single `detector.detect(gray_frame)` call returns tag ID + 3D pose. For the capstone's slow manipulation tasks, 8 FPS is more than sufficient.
