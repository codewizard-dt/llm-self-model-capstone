...
| Telemetry (out) | REPO `v5-test/src/main.cpp`, `robot/pi-runtime/docs/BRAIN_INTERFACE.md §2` (VERIFIED) | Emit **newline-delimited JSON** on the USER port with **COBS disabled** (`serctl(SERCTL_DISABLE_COBS)`), `printf`+`fflush`. ~100 Hz during an action; silent between. | confirmed |
