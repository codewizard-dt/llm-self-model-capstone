...
| D6 | Environment model | **Per-stack uv projects, NOT a root workspace.** The Tech-stack registry pins different Python versions per vertical (`contracts`/`operator` = 3.12; `pilot`/`coprocessor` = 3.11). A uv workspace has a single shared interpreter and cannot host both, so each vertical is an **independent uv project** with its own `.python-version` and isolated `.venv` (matches conventions: "each vertical is its own Stack/Factory"). The root uv workspace created in `package-scaffold` is removed by the `per-stack-venv` slice. `contracts` tightens to `requires-python = ">=3.12,<3.13"`. *(Supersedes the D7 "root uv workspace" decision recorded in the `package-scaffold` plan artifact.)* | `closed` |
| D7 | Build gate routing | **`changeset.build` routes through `make sync`, not raw root `uv sync`.** With no shared root environment, the build gate must be stack-scoped like `test`/`validate`/`lint`: the root `Makefile` `sync` target delegates to the slice's stack (`cd <stack> && uv sync`), so "build" means "sync this stack's env on this stack's Python." Applied by the `per-stack-venv` slice; this is a factory-wide check change consistent with the existing make-delegated gates. | `closed` |
