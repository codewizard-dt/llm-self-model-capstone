...
- `SelfModel` pydantic v2 model in `contracts/src/contracts/self_model.py`, re-exported as `from contracts import SelfModel`:
  ...
  - `reasoning: dict[str, str]` — required, non-empty map with non-empty values; one keyed rationale per change/choice (PR #13 review, D9). Gen 0 keys by its initial structural choices; a revision keys by each changed parameter.
...
| D9 | Vocabulary + reasoning revision (PR #13 review) | Dial the config grammar back to feasible options and make `reasoning` per-change: `EndEffector` = `claw_grasper`/`scoop`/`flywheel` (drop bare_arm/none); `ArmPosition` trimmed to `front`/`rear`; `Cartridge` = `100rpm`/`200rpm`/`600rpm`; `reasoning` becomes `dict[str, str]` (one rationale per change/choice). Ripples to `vocabulary.py`, `self_model.py`, fixtures, `self_model.json`, the `.ai-sdd` self-model seed schema + example, and the parts-catalog seed example. `MASTER_REQUIREMENTS.md` is reconciled separately (owner is editing it on `main`). | `closed` |
