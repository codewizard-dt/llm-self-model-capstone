...
| Depends on | F1 telemetry-contract, F2 self-model-schema, F3 parts-catalog grammar, F4 adapter-interfaces |
| Blocked for full implementation | F10 gap-analyzer |
...
F3 parts-catalog grammar has landed, so the Generator work may consume
`contracts.PartsCatalog`, `contracts/parts_catalog.json`, and
`contracts.validate_config` for finite vocabulary and buildability checks. Full
F8 implementation remains blocked until F10 provides the signed gap analyzer
residual summary. Until F10 lands, gap-specific work must expose missing inputs
as blocked or fixture-backed instead of inventing field names, telemetry shapes,
or residual keys.

## In Scope

- F8 support packet planning for an operator packet builder that collects
  `contracts.ContractLine`, `contracts.SelfModel`, shared vocabulary,
  `contracts.PartsCatalog` / `contracts.validate_config` buildability
  constraints, human constraints, blocked-state notes, and future F10 gap
  summaries.
...
- Changing F3 parts catalog valid-config rules.
...
- Shared vocabulary and buildability rules come from `contracts.vocabulary`,
  `contracts.PartsCatalog`, `contracts/parts_catalog.json`, and
  `contracts.validate_config`.
...
F10 gap analyzer is the remaining blocker for full F8. Parts-catalog
buildability must come from F3's committed contract surface, not from
operator-local rules. Fixture summaries are allowed only when clearly labeled
fixture-backed.
...
3. The plan states F10 gap-analyzer is a dependency and blocker for full
   implementation.
4. The plan states F3 parts-catalog grammar is a landed dependency and F10 gap
   analyzer is the remaining blocker for full F8.
