...
- **`contracts/src/contracts/control_command.py`**, re-exported from `contracts.__init__`:
  ...
  - **`AckLine`** — `type: Literal["ack"]`, `v: Literal[1]`, `ack: int` (the `seq` it acknowledges), `state: Literal["ok", "rejected", "stale"]` (D6), `recv_ms: int`, `fault: FaultCode | None` (model-validator: `fault is None` iff `state == "ok"`). **No** telemetry-leaking fields (D3).
  ...
  - **Clamp constants** (module-level): `MAX_LINEAR=0.35`, `MAX_OMEGA=0.60`, `MAX_FLYWHEEL_RPM=3600`, `ARM_DEG_MIN=0`, `ARM_DEG_MAX=360`, `TTL_MS_MAX=1000`. The single source F20 imports (D10) — reconciles the inline C++ literals in `pros_bridge/src/main.cpp` (C1).
- **`contracts/src/contracts/__init__.py` extended additively** — add `ControlLine`, `ControlCommand`, `ControlEnvelope`, `HeartbeatLine`, `AckLine`, `StopCommand`, `DriveCommand`, `TurnCommand`, `ArmCommand`, `ClawCommand`, `FlywheelCommand`, `FaultCode`, `MAX_LINEAR`, `MAX_OMEGA`, `MAX_FLYWHEEL_RPM`, `ARM_DEG_MIN`, `ARM_DEG_MAX`, `TTL_MS_MAX` to imports and `__all__`. All existing exports unchanged.
...
| D3 | Ack field set | **Receipt-only — `state` + `fault`.** No `battery_mv`/`heading_deg`/etc. — telemetry belongs to `ContractLine` (F1). | `closed` |
...
| D5 | Wire bounds | **`ttl_ms: Field(ge=1, le=1000)`** (matches existing `bridge.py` clamp; gives F20's `WATCHDOG_MS=250` headroom). **`seq: Field(ge=0, lt=2**32)`** (matches a future binary framing). | `closed` |
...
| D10 | Clamp constants — where they live | **Exported from `contracts.control_command`**: `MAX_LINEAR=0.35`, `MAX_OMEGA=0.60`, `MAX_FLYWHEEL_RPM=3600`, `ARM_DEG_MIN=0`, `ARM_DEG_MAX=360`, `TTL_MS_MAX=1000`. F19 enforces them as `Field(ge=…, le=…)` bounds **and** exposes them as module constants so F20 imports the same numbers (single source — Acceptance §5 identity test). | `closed` |
