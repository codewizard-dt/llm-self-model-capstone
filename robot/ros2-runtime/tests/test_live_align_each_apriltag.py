...
# (method, args, kwargs, timeout_s)  — all steps are nav steps
STEPS = [
    ("locate_nearest_apriltag", [], {}, 20.0),
    ("orient_to_tag", [0], {}, 15.0),
    ("move_to_tag", [0], {"target_distance_m": TARGET_DISTANCE_M}, 30.0),
    ("orient_to_tag", [1], {}, 15.0),
    ("move_to_tag", [1], {"target_distance_m": TARGET_DISTANCE_M}, 30.0),
    ("orient_to_tag", [2], {}, 15.0),
    ("move_to_tag", [2], {"target_distance_m": TARGET_DISTANCE_M}, 30.0),
]
...
class AlignEachTagTestNode(Node):
    ...
    def run(self) -> bool:
        ...
        for method, args, kwargs, timeout_s in STEPS:
            ...
            print(f"[{method}{args if args else ''}]{kw_str}")
            ...
            while time.monotonic() < deadline:
                ...
                if result.success:
                    tag_index = args[0] if args else None
                    if method == "move_to_tag" and tag_index is not None and result.tag:
                        dist = float(result.tag.distance_m or 0.0)
